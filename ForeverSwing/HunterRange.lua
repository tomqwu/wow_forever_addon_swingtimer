...
function Range.Create(host, db)
    ...
    if not Core.IsReadable(class) or class ~= 'HUNTER' then return end
    local frame = CreateFrame('Frame', 'ForeverSwingHunterRange', host)
    frame:SetSize(300,24)
    frame:SetPoint('TOPLEFT',host,'BOTTOMLEFT',0,-6)
    ...
    local function Refresh()
        ...
        frame:SetShown(db.hunterrange)
        if not db.hunterrange then return end
