...
class PublisherTests(unittest.TestCase):
    ...
    def test_zip_tag_and_interface(self):
        with tempfile.TemporaryDirectory() as folder:
            archive = Path(folder)/'ForeverUtilities-0.2.0.zip'
            with zipfile.ZipFile(archive,'w') as z:
                z.writestr('ForeverUtilities/ForeverUtilities.toc', '## Interface: 16001\n## Version: 0.2.0\n')
            self.assertEqual(publisher.read_package(archive,'ForeverUtilities-v0.2.0'),'1.60.1')
            with self.assertRaises(ValueError):
                publisher.read_package(archive,'ForeverUtilities-v0.3.0')
    ...
    def test_rejects_mixed_archive(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with self.assertRaises(ValueError):
                publisher.read_package(archive, 'ForeverUtilities-v0.1.1')
