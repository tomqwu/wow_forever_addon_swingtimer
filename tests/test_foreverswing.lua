...
local function run(blocked, saved, initOnly)
    ...
    if initOnly then return ForeverSwingDB.cue, ForeverSwingDB.cuePolicyVersion end
    ...
    check(not main.textures[2].shown and not main.textures[3].shown, 'no default twist window on real swing')
    SlashCmdList.FOREVERSWING('cue 0.4')
    host.scripts.OnUpdate(host, 0.1)
    check(main.textures[2].shown and main.textures[3].shown, 'explicit reference zone and line visible')
...
local cue, policy = run(false, {cue=0}, true)
check(cue == 0 and policy == 1, 'legacy zero remains disabled')
cue = run(false, {cue=0.4}, true)
check(cue == 0, 'legacy 0.4 default removed')
cue = run(false, {cue=0.4, cueConfigured=true}, true)
check(cue == 0, '0.2.1 forced cue corrected')
cue = run(false, {cue=0, cuePolicyVersion=1}, true)
check(cue == 0, 'disabled cue survives reload')
cue = run(false, {cue=0.4, cuePolicyVersion=1}, true)
check(cue == 0.4, 'explicit post-migration reference survives reload')
cue = run(false, {cue=0.7}, true)
check(cue == 0.7, 'custom nonzero cue preserved')
cue = run(false, {}, true)
check(cue == 0, 'new install has no twist window')
run(false)
run(true)
